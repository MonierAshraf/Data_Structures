	...
	int rearQueue()
	{
		assert(!isEmpty());// assert function take boolean vaiable and if it's false it stop the program a
		//and give the line number that introduce this error
		return arr[rear];
	}
	...
	void printQueue()
	{
		if (!isEmpty()) {
			...
			cout << arr[rear];// as loop will not cover the last element
		}
		else
			cout << "Empty Queue";
	}
